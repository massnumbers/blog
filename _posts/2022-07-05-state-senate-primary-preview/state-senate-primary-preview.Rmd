...
sen_cand <- read_excel("../../_data/2020/MA_State_Senate_Race_2022.xlsx", sheet="Contested") %>%
...
The First Essex State Senate District—one of the new majority districts—contains
all of Lawrence, all of Methuen, and about 5 precincts of Haverhill. The
previous lines of the district left out Lawrence and included a large number of
largely white communities. The new lines result in a voting age population that
is 36% White, 56% Hispanic, 10% Black, and 3% Asian. The First Essex incumbent,
Diana DiZoglio (D—Methuen), is running for Auditor, leaving an open race for a
competitive field of candidates from Lawrence and Methuen (see the First Essex
primary overview below).
...
ma_sen_cands_1 <- tibble(
    candidate=c("Paul W. Mark", "Huff Tyler Templeton III", "Brendan M. Phair"),
    party=c("Democrat", "Democrat", "Unenrolled"),
    city_town=c("Peru", "Williamstown", "Pittsfield")) %>%
    merge((ma_towns_1 %>% select(city_town, center)), by="city_town") %>%
    select(-geometry) %>%
    st_as_sf(sf_column_name="center") %>%
    relocate(candidate)
```
...
ma_full_towns_4 <- ma_4_completes %>%
    filter(complete) %>%
    select(city_town) %>%
    left_join(ma_towns_4, by="city_town") %>%
    select(name=city_town, PVI, white_pct, black_pct,
           hispanic_pct, asian_pct, geometry)

ma_precincts_4 <- ma_4_completes %>%
    filter(!complete) %>%
    select(city_town) %>%
    left_join(ma_vtds_4, by="city_town") %>%
    mutate(name = str_c(city_town, Ward, Pct, sep=" ")) %>%
    select(name, PVI, white_pct, black_pct, hispanic_pct, asian_pct, geometry)

## ma_towns_pcts_4 <- rbind(ma_full_towns, ma_precincts_4)) %>%
##     st

ma_house_hampd_7 <- ma_house_12 %>%
    filter(DIST_CODE == "HAMPD07")

ma_sen_cands_4 <- tibble(
    candidate=c("Sydney R. Levin-Epstein",
                "Jacob R. Oliveira",
                "John Harding",
                "William E. Johnson"),
    party=c("Democrat", "Democrat", "Republican", "Republican"),
    city_town=c("Longmeadow", "Ludlow", "East Longmeadow", "Granby")) %>%
    merge((ma_towns_4 %>% select(city_town, center)), by="city_town") %>%
    select(-geometry) %>%
    st_as_sf(sf_column_name="center") %>%
    relocate(candidate)
```
...
## ma_sen_4_map <- (tm_shape(ma_towns_4, bbox=st_bbox(st_union(ma_sen_4, ma_house_hampd_7))) +
##                  tm_borders(lwd=0.5) + tm_fill(col="PVI_N",
##                                                palette=c("red", "white", "blue"),
##                                                midpoint=0,
##                                                alpha=0.5,
...
##                  tm_symbols(ymod=-0.6, size=0.2, col="black", popup.vars=c("party", "city_town")) +
##                  tm_text(ymod=-1.5, text="candidate", size=0.5) +
##                  tm_basemap("OpenStreetMap"))
