...
```{r partisan-shifts-map, fig.height=5, fig.cap="State Senate partisan shifts 2012-2020"}
#| column: body-outset
senate_shift_map <- (
 ...
 tm_fill(col="shift_12_20",
         ...
         midpoint=0,
         alpha=0.8,
         popup.vars=c("dem_pct_12", "dem_pct_16", "dem_pct_20", "shift_12_20")) +
...
bm2 <- stan_glm(dem_win ~ PVI_N + incumbent_status + pres_year, data=ma_leg, family=binomial(link="logit"), refresh=0, seed=1234)
blm2 <- stan_glm(dem_percent ~ PVI_N + incumbent_status + pres_year, data=ma_leg, family=gaussian(link="identity"), refresh=0, seed=1234)
```
...
```{r nbm-nwm-plot, fig.cap="Norfolk, {Bristol, Worcester}, Middlesex model vs. outcome"}
#| column: body-outset
